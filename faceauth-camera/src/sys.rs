...
//!
//! The ioctl wrappers below are generated by `nix`'s macros. Each takes a
//! file descriptor and a pointer to one of the `repr(C)` structs here; the
//! caller's obligation, stated once here and referenced at each call site,
//! is that the fd is open and the struct is the one the ioctl number names,
//! so the kernel reads and writes exactly the bytes the struct spans.
...
/// `V4L2_BUF_FLAG_ERROR`: the driver dequeued the buffer but its payload is
/// not a whole frame (a transfer error); the data is not to be read.
pub const V4L2_BUF_FLAG_ERROR: u32 = 0x40;
...
// Media-bus codes from `linux/media-bus-format.h`. The four 10-bit Bayer
// codes are not contiguous: 0x3009 is `SGRBG10_DPCM8_1X8`, a compressed
// format, and the GBRG one is 0x300e.
...
impl v4l2_format {
    ...
    // The four views below share one argument: both member structs are
    // repr(C) and all-integer, so every bit pattern is a valid value; they
    // are smaller than the 200-byte union (48 and 192 bytes, pinned by
    // `layout_tests`); and `raw` starts at offset 8 of a 4-aligned struct,
    // which satisfies both members' 4-byte alignment.
    pub fn pix(&self) -> &v4l2_pix_format {
        // SAFETY: see the note above the accessors.
...
/// Arrays longer than 32 keep this and `media_entity_desc` off `derive(Default)`;
/// `zeroed()` builds them, which is sound for a struct of integers.
#[repr(C)]
#[derive(Clone, Copy)]
...
impl media_device_info {
    pub fn zeroed() -> Self {
        // SAFETY: every field is an integer or an array of integers, for
...
mod layout_tests {
    ...
    /// The Bayer codes are pinned by value because two of them were once
    /// wrong: 0x3009 is a DPCM8 code, not GBRG.
    #[test]
    fn media_bus_codes_match_the_header() {
        assert_eq!(MEDIA_BUS_FMT_Y10_1X10, 0x200a);
        assert_eq!(MEDIA_BUS_FMT_SBGGR10_1X10, 0x3007);
        assert_eq!(MEDIA_BUS_FMT_SGRBG10_1X10, 0x300a);
        assert_eq!(MEDIA_BUS_FMT_SGBRG10_1X10, 0x300e);
        assert_eq!(MEDIA_BUS_FMT_SRGGB10_1X10, 0x300f);
        assert_ne!(
            MEDIA_BUS_FMT_SGBRG10_1X10, 0x3009,
            "0x3009 is SGRBG10_DPCM8_1X8"
        );
