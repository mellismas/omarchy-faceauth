...
impl Watch {
    ...
    pub(crate) fn locked_step(
        &mut self,
        now: Instant,
        matched_since_lock: bool,
        ...
        self.state = if matched_since_lock {
            State::Present
        } else {
            State::Unknown
        };
...
pub fn run(auth: Arc<Mutex<Authenticator>>, cfg: PresenceConfig) {
    ...
    log::info!(
        "presence watch on for {} (mode {}, tick {}s, away after {}s, attention {})",
        cfg.user,
        cfg.mode.name(),
        cfg.tick_seconds,
        cfg.away_seconds,
        cfg.require_attention
    );
    let mut on_battery = false;
    let mut mode = presence_mode();
    loop {
        let (tick_s, every) = if on_battery && cfg.battery_tick_seconds > 0.0 {
            (cfg.battery_tick_seconds, cfg.battery_identify_every)
        } else {
            (cfg.tick_seconds, cfg.identify_every)
        };
        ...
        let identify = tick.is_multiple_of(every.max(1))
            || (w.state != State::Present && tick.is_multiple_of(2));
        ...
        if let Some(t) = w.locked_at {
            let matched = auth
                .lock()
                .map(|a| a.last_match.get(&cfg.user).map(|m| *m > t).unwrap_or(false))
                .unwrap_or(false);
            let Some(how) = w.locked_step(Instant::now(), matched, &|| {
        ...
        if let Ok(json) = serde_json::to_string(&pub_) {
            ...
            if written
                .and_then(|_| std::fs::rename(&tmp, &cfg.state_file))
                .is_err()
            {
...
fn run_lock_command(cfg: &PresenceConfig) -> Result<()> {
    let Some(bin) = cfg.lock_command.first().filter(|b| !b.is_empty()) else {
        anyhow::bail!("no lock command configured")
    };
...
pub(crate) fn observe(
        ...
        .and_then(|m| m.for_face(&img, &face).ok().flatten())
    {
        Some(m) => faceauth_engine::mesh::is_attentive(
            ...
            cfg.max_yaw * crate::consent::NodDetector::YAW_DEG_PER_UNIT,
            cfg.max_roll_degrees,
        ),
...
fn strobe_pair(
    ...
    while t0.elapsed() < Duration::from_millis(600) {
        let Some(img) = cap.next(Duration::from_millis(200))? else {
            continue;
        };
...
mod watch_tests {
    ...
    fn a_stranger_in_view_locks_at_away_seconds_in_both_modes() {
        for mode in [PresenceMode::Default, PresenceMode::Secure] {
            ...
            let at = first_lock(&mut w, t0, &looks, mode)
                .unwrap_or_else(|| panic!("{:?}: a stranger held the lock off", mode));
            ...
            assert_eq!(
                at,
                if mode == PresenceMode::Secure { 5 } else { 6 },
                "{:?}",
                mode
            );
    ...
    fn one_failed_check_is_enough_only_in_the_secure_mode() {
        ...
        assert_eq!(
            ...
            None,
            "default: one failed check, then unconfirmed faces, still counts as the user"
        );
        ...
        assert_eq!(
            ...
            Some(5),
            "secure: the clock ran from the last good sighting at tick 1"
        );
    ...
    fn a_hidden_face_holds_the_clock_only_in_the_default_mode_and_only_with_a_shape() {
        ...
        assert_eq!(
            first_lock(&mut w, t0, &looks, PresenceMode::Secure),
            Some(5),
            "secure: nothing but the user's face holds it"
        );
    ...
    fn the_watch_resumes_after_an_unlock_without_a_face() {
        ...
        for _ in 0..(LOCK_CHECK_TICKS * 2) {
            assert_eq!(
                w.locked_step(now(), false, &still_locked),
                None,
        ...
        assert_eq!(
            w.locked_step(resume_at + Duration::from_secs(65), true, &still_locked),
            Some("by face")
        );
        assert_eq!(w.state, State::Present);
    }
